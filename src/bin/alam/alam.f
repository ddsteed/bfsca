...
C * MAR 01 1979 - MAM - CALCULATE MO CHARGE DISTRIBUTION FOR A LINEAR MOLECULE
      ...
      PARAMETER (NGMX=270,NTMX=35,NCMX=35,NCBFNX=220,NOCCX=25)
...
C *
C * NQANGMX :== MAX # OF ANGLES IN THE THETA QUADRATURE. (32)
C * NRPTSX  :== MAX # OF POINTS IN THE R-MESH (500)
C * NNUCMX  :== MAX # OF NUCLEI THAT THE CODE WILL HANDLE (3)
C *             (THIS IS ALSO THE MAX. # OF CENTERS FOR A STO
C *             CALCULATION, BUT NOT FOR A GTO CALCULATION.)
C * NSTOMX  :== MAX # OF STO FCNS. ON ONE CENTER. (12)
C * LAMMX   :== MAX VALUE OF LAMDA FOR THE LEGENDRE FCNS. STORED
C *             IN THE BLOCK DATA TABEL (40)
C * NTLAMX  :== MAX # OF LAMDAS IN THE ARRAYS USED WHEN ITEST=1 (21)
C * NLAMMX  :== MAX # OF LAMDAS IN THE ARRAYS USED WHEN ITEST .NE. 1 (41)
C * NGMX    :== MAX # OF GAUSSIAN PRIMITIVES (125)
C * NCBFNX  :== MAX # OF CONTRACTED GAUSSIANS (80)
C * NOCCX   :== MAX # OF OCCUPIED ORBITALS (11)
C * NCMX    :== MAX # OF CENTERS IN THE XZ-PLANE FOR GTOS (20)
C * NTMX    :== 20 (DON'T CHANGE THIS ONE)
C *-/
...
C     CHARACTER*4 ILBL,ICENTR
     ...
     $            2,1,1,2 /
C
      DATA  ITYPE/'  S ','  X ','  Y ','  Z ',' XX ',' YY ',' ZZ '
     $     ,' XY ',' XZ ',' YZ ',' XXX',' YYY',' ZZZ',' XXY',' XXZ'
     $     ,' YYX' ,' YYZ',' ZZX',' ZZY',' XYZ','XXXX','YYYY','ZZZZ'
     $     ,'XXXY','XXXZ','YYYX','YYYZ','ZZZX','ZZZY','XXYY','XXZZ'
     $     ,'YYZZ','XXYZ','YYXZ','ZZXY'/
...
C *
      ...
      CALL   GINTS (NTYPE,NR,NFIRST,NLAST,ETA,NCBFNX,NTMX, NGMX ,NCBFN
     $     ,NOCC,NPRNT)
...
C *
     ...
     $     ,NON,LPSKIP, NOCC)
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
      CHARACTER*4 ITYPE(NTXX), KTYP, IBLNK, IJLK, JKPR
...
670   WRITE (6,671)
      ...
      WRITE (6,672) NBFXX,NGXX

      WRITE (6,673) NCXX

      WRITE (6,674)
 671  FORMAT(//5X,'PROGRAM LIMITATIONS')
 672  FORMAT(/5X,'MAX NO BASIS FUNCTIONS=   ',I4 /5X
     $     ,'MAX NO GAUSSIAN PRIMATIVES=    ',I4)
 673  FORMAT(/5X,'MAX NO CENTERS=  ',I4)
 674  FORMAT(/5X,  'S,P,D,F,G TYPE GAUSSIANS ONLY')
...
C910  FORMAT (A4, 6X, A4, I3, 3X, 2F12.0)
C910  FORMAT (I2, 3X, A3, I3, 3X, F12.6, 2X, F12.6)
 910  FORMAT (I2, 2X, A4, I3, 2X, F13.6, X, F13.6)
C1911 FORMAT(3(3X,I5,3X), 4X,A4,5X,A4,2F15.7,2X,3A8)
 1911 FORMAT(3(3X,I5,3X), 4X,I4,6X,A4,2F15.7,2X,3A8)
 915  FORMAT (36I2)
...
C *-
     ...
     $     ,NOCC,NPRNT)
      ...
      INTEGER OUT
      PARAMETER (NOCCX=25,NCBFNX=220,OUT=6)
      PARAMETER (ZERO=0.0D0,ONE=1.0D0)
